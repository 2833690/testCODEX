...
def compute_metrics(initial_cash: float, final_equity: float, equity_curve: list[float], trades: list[TradeRecord]) -> dict[str, float]:
    ...
    downside_dev = (fmean([r**2 for r in downside]) ** 0.5) if downside else 0.0
    vol = (fmean([(r - avg_ret) ** 2 for r in returns]) ** 0.5) if returns else 0.0
    sharpe_like = (avg_ret / vol) * (len(returns) ** 0.5) if vol > 0 else 0.0
    sortino_like = (avg_ret / downside_dev) * (len(returns) ** 0.5) if downside_dev > 0 else 0.0
    ...
    calmar_like = net_return / max_dd if max_dd > 0 else 0.0
...
def rank_strategies(results: dict[str, dict[str, float]]) -> list[tuple[str, float]]:
    ...
    for name, metrics in results.items():
        score = (
            metrics.get("sortino_like", 0.0) * 0.35
            + metrics.get("sharpe_like", 0.0) * 0.25
            + metrics.get("calmar_like", 0.0) * 0.2
            + metrics.get("net_return", 0.0) * 0.2
            - metrics.get("max_drawdown", 0.0) * 0.25
        )
