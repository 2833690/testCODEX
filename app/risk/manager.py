...
class RiskManager:
    ...
    def approve(
        ...
        if risk_state.daily_pnl < -(equity * self.settings.max_daily_loss_pct):
            return RiskDecision(approved=False, reason="daily_loss_limit")
        if risk_state.consecutive_losses >= self.settings.consecutive_losses_limit:
        ...
        if len(candles) >= 20:
            ...
            if volatility > self.settings.max_volatility_pct:
                return RiskDecision(approved=False, reason="volatility_too_high")

        size = self.position_size(equity=equity, entry=market.last, stop_loss=signal.stop_loss)
        if size <= 0.0:
